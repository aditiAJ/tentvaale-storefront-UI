...
import { SOCIAL_LINKS, type SocialId } from "@/lib/company";

/**
 * Brand marks for the social links.
 *
 * lucide-react dropped brand icons at v1, so these are inlined rather than
 * imported. Each is a single filled path on a 24×24 box, sized by the parent's
 * font-size-independent `size-*` class.
 */
const PATHS: Record<SocialId, string> = {
  ...
  youtube:
    "M23.5 6.19a3.02 3.02 0 0 0-2.12-2.14C19.5 3.54 12 3.54 12 3.54s-7.5 0-9.38.51A3.02 3.02 0 0 0 .5 6.19C0 8.08 0 12 0 12s0 3.92.5 5.81a3.02 3.02 0 0 0 2.12 2.14c1.88.51 9.38.51 9.38.51s7.5 0 9.38-.51a3.02 3.02 0 0 0 2.12-2.14C24 15.92 24 12 24 12s0-3.92-.5-5.81M9.55 15.57V8.43L15.82 12l-6.27 3.57",
...
/**
 * The row of social links used in the footer and on the contact page.
 * `tone` picks the border weight so it reads correctly on both surfaces.
 */
    ...
    <div className={cn("flex items-center gap-2.5", className)}>
      {SOCIAL_LINKS.map(({ id, label, href }) => (
        <a
          ...
          title={label}
          className={cn(
            "flex items-center justify-center rounded-md border border-border text-muted-foreground transition-colors duration-200 ease-out-quint hover:border-primary hover:text-primary",
            size === "sm" ? "size-8" : "size-9"
          )}
        >
          <SocialIcon id={id} className={size === "sm" ? "size-3.5" : "size-4"} />
        </a>
      ))}
    </div>
  );
